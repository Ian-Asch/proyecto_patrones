...
ALTER TABLE
    `Packages` ADD CONSTRAINT `packages_clientid_foreign` FOREIGN KEY(`ClientID`) REFERENCES `Users`(`UserID`) ON DELETE CASCADE;
ALTER TABLE
    `Packages` ADD CONSTRAINT `packages_routeid_foreign` FOREIGN KEY(`RouteID`) REFERENCES `Routes`(`RouteID`) ON DELETE CASCADE;
ALTER TABLE
    `RouteTransportUnit` ADD CONSTRAINT `routetransportunit_unitid_foreign` FOREIGN KEY(`UnitID`) REFERENCES `TransportUnits`(`UnitID`);
ALTER TABLE
    `RouteTransportUnit` ADD CONSTRAINT `routetransportunit_routeid_foreign` FOREIGN KEY(`RouteID`) REFERENCES `Routes`(`RouteID`) ON DELETE CASCADE;
...
create
    definer = admin@localhost procedure update_Package(
    IN in_userID int,
    IN in_name VARCHAR(255),
    IN in_identificationNumber  VARCHAR(100),
    IN in_companyName VARCHAR(255),
